    ...
    char **argv)
{
    ...
    fpx_list_push_back(&params->server->listeners, &listener->list_node);
    ...
    char **argv)
{
    ...
    if (level == FPX_LOG_INVALID_LEVEL) {
        fpx_log_error1(FPX_LOG_ERROR, FPX_ERR_CONFIG_SYNTAX,
            "%s:%zu: \"%s\" is unknown log level", params->directive->filename,
            params->directive->line, argv[1]);
        return FPX_ERR_CONFIG_SYNTAX;
    }
...
fpx_core_preinit(fpx_server_t *server)
{
    ...
    fpx_assert(fpx_logger_init(&server->logger) == FPX_OK);
...
fpx_core_init(fpx_server_t *server, void *data)
{
    ...
    if (fpx_list_size(&server->listeners) == 0) {
        fpx_log_error1(FPX_LOG_ERROR, FPX_FAILED,
            "Listen address and port not specified");
        return FPX_FAILED;
    }

    fpx_list_foreach (node, &server->listeners) {
        listener = fpx_list_data(node, fpx_listener_t, list_node);
