    ...
    fpx_uint_t line)
{
    ...
    fpx_list_push_back(&config->conf_list, &directive->list_node);
    ...
    for (i = 0;; ++i) {
        ...
        if (found) {
            ...
            if (argc >= FPX_MAX_TAKES) {
                fpx_log_error1(FPX_LOG_ERROR, FPX_ERR_CONFIG_SYNTAX,
                    "%s:%zu: Too many arguments in directive \"%s\"", filename,
                    line, str);
                return FPX_ERR_CONFIG_SYNTAX;
            }
    ...
    const char *filename)
{
    ...
    fpx_list_push_back(&config->conf_files, &file->list_node);
...
fpx_config_read(fpx_config_t *config, fpx_pool_t *pool, const char *filename)
{
    ...
    fpx_list_init(&config->conf_list);
...
fpx_config_check(fpx_config_t *config, fpx_server_t *server)
{
    ...
    fpx_list_foreach (node, &config->conf_list) {
        directive = fpx_list_data(node, fpx_directive_t, list_node);
