#ifndef FPX_CORE_LOG_H
#define FPX_CORE_LOG_H
...
#define fpx_log_stdout(err, ...) \
    fpx_log_console(FPX_STDOUT, err, __VA_ARGS__)

#define fpx_log_stderr(err, ...) \
    fpx_log_console(FPX_STDERR, err, __VA_ARGS__)
...
#define fpx_assert(exp) { \
    if (fpx_logger_get_global()) { \
        if (!(exp)) { \
            fpx_log_error(FPX_LOG_EMERG, fpx_logger_get_global(), \
                FPX_OK, "%s: Assertion \"%s\" failed", FPX_FILE_LINE, \
                fpx_stringify(exp)); \
            abort(); \
        } \
    } \
...
void fpx_log_error(fpx_uint_t level, fpx_logger_t *logger,
    fpx_err_t err, const char *fmt, ...);
