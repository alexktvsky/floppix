...
#define fpx_snprintf(str, size, ...) \
    snprintf(str, size, __VA_ARGS__)
#define fpx_vsnprintf(str, size, ...) \
    vsnprintf(str, size, __VA_ARGS__)

#define fpx_isdigit(c) \
    ((c >= '0') && (c <= '9'))

#define fpx_isalpha(c) \
    (((c >= 'A') && (c <= 'Z')) || ((c >= 'a') && (c <= 'z')))

#define fpx_isascii(c) \
    ((((int8_t) c) >= 0x0) && (((uint8_t) c) <= 0x7f))

#define fpx_tolower(c) \
    ((char) ((c >= 'A' && c <= 'Z') ? (c | 0x20) : c))

#define fpx_toupper(c) \
    ((char) ((c >= 'a' && c <= 'z') ? (c & ~0x20) : c))
...
fpx_int_t fpx_atoi(const char *str, size_t n);
...
fpx_int_t fpx_strncasecmp(char *str1, char *str2, size_t n);
