#ifndef FLOPPIX_SYSTEM_POOL_H
#define FLOPPIX_SYSTEM_POOL_H
...
fpx_err_t fpx_pool_create(fpx_pool_t **pool, size_t size, fpx_pool_t *parent);
void fpx_pool_add_child(fpx_pool_t *parent, fpx_pool_t *new_child);
void *fpx_palloc1(const char *filename, int line, fpx_pool_t *pool,
    size_t size);
void *fpx_pcalloc1(const char *filename, int line, fpx_pool_t *pool,
    size_t size);
