...
status_t init_log(char *filename, int level_boundary) {
    ...
    pthread_attr_init(&log_tattr);
    pthread_attr_setscope(&log_tattr, PTHREAD_CREATE_DETACHED);

    if (pthread_create(&log_tid, NULL, logger, (void *) filename)) {
        return 2;
    }
...
status_t log_message(int level, char *message) {
    ...
    message_unit_s *node = palloc(pool, sizeof(message_unit_s));
